...
| A traditional bruteforce alorithm                                              | The optimized bruteforce algorithm                                             |
| ------------------------------------------------------------------------------ | ------------------------------------------------------------------------------ |
...
possibility, it there is nothing to backtrack. Except it happens with every guess of the
...
It has 5 possible values, so each guess for that field has a change of 20%. Meanwhile, the improved
...
case for the solver. And if the solvers were just iterating over same tree of possibilities just
restructured a bit, we would expect the number of iterations to be similar.
